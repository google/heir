...
class BuildBazelExtension(build_ext.build_ext):
  ...
  def copy_include_files(self):
    ...
    # Some headers (e.g. config_core.h) are generated by Bazel genrules and
    # live in bazel-bin rather than the source tree. Merge those in, but skip
    # files already present (the main copytree may have picked them up via
    # execroot symlinks, and those copies are read-only).
    def copy_if_missing(src, dst):
      if not os.path.exists(dst):
        shutil.copy2(src, dst)

    bazel_bin_external = Path(self.build_temp) / "bazel-bin" / "external"
    for subdir in subdirs:
      src = bazel_bin_external / subdir
      if src.exists():
        print(f"Copying generated headers from {src} to {libdir / subdir}")
        shutil.copytree(
            src=src,
            dst=libdir / subdir,
            ignore=include_patterns(patterns),

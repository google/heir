...
from heir.backends.dummy import DummyBackend
from heir.core import ClientInterface, BackendInterface
...
def run_pipeline(
    ...
    heir_config: HEIRConfig = heir_cli_config.DEVELOPMENT_HEIR_CONFIG,
    debug: bool = False,
  ...
  try:
    ...
    try:
        ...
        print(Fore.RED + Style.BRIGHT + f"HEIR Error: Signature parsing failed for function {func_name} with {type(e).__name__}: {e}")
        raise
    try:
        fn_args, _ = sigutils.normalize_signature(numba_signature)
    except Exception as e:
        print(Fore.RED + Style.BRIGHT + f"HEIR Error: Signature normalization failed for  function {func_name} with signature {numba_signature} with {type(e).__name__}: {e}")
        raise
    ...
    try:
        ...
        print(Fore.RED + Style.BRIGHT + f"HEIR Error: Type inference failed for function {func_name} with signature {numba_signature} with {type(e).__name__}: {e}")
        raise

    # Emit Textual IR
    mlir_textual = TextualMlirEmitter(ssa_ir, secret_args, typemap, restype).emit()
    if(debug):
    ...
    if(debug):
        ...
        print("HEIR Debug: " + Style.BRIGHT + f"Running heir-opt {' '.join(heir_opt_options)}")
    heir_opt_output, graph = heir_opt.run_binary_stderr(
        input=mlir_textual,
    ...
    result = backend.run_backend(workspace_dir, heir_opt, heir_translate, func_name, arg_names, secret_args, heir_opt_output, debug)
...
def compile(
    ...
    backend: Optional[BackendInterface] = OpenFHEBackend(openfhe_config.DEFAULT_INSTALLED_OPENFHE_CONFIG),
    heir_config: Optional[heir_cli_config.HEIRConfig] = heir_cli_config.from_os_env(),
  ...
  Args:
      scheme: a string indicating the scheme to use. Options: 'bgv' (default),
        'ckks'.
      backend: backend object (e.g. OpenFHEBackend) to use for running the function
  ...
  def decorator(func):
    return run_pipeline(
        func,
        heir_opt_options=heir_opt_options or [f"--mlir-to-{scheme}"],

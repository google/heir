...
struct ConvertLinalgConv2DNchwFchw
 ...
 public:
  using ContextAwareOpConversionPattern<
      linalg::Conv2DNchwFchwOp>::ContextAwareOpConversionPattern;
...
struct RotomTensorOpLowering {
  ...
  FailureOr<std::vector<std::vector<int64_t>>> getRangePointsForDomain(
      LayoutAttr layout, ArrayRef<int64_t> domain) const {
  ...
  FailureOr<DenseIntElementsAttr> createRemapAttr(
    ...
    auto type =
        RankedTensorType::get({static_cast<int64_t>(values.size() / 4), 4},
  ...
  FailureOr<Value> alignDomainPointToOutput(
    ...
    setAttributeAssociatedWith(remap.getResult(), kLayoutAttrName,
                               outputLayout);
  ...
  LogicalResult lowerMatmul(
      ...
      ContextAwareConversionPatternRewriter& rewriter) const {
    LLVM_DEBUG(llvm::dbgs()
               << "Converting linalg.matmul op with Rotom matmul kernel: " << op
               << "\n");
  ...
  LogicalResult lowerElementwiseBinary(
    ...
    auto ciphertextSemanticType =
        dyn_cast<RankedTensorType>(convertedResultType);
    ...
    if (!isAdd && !isMul) {
      return rewriter.notifyMatchFailure(
          op, "unsupported Rotom elementwise op kind");
    }
    ...
    iterateIndices(
        resultType.getShape(), [&](const std::vector<int64_t>& domain) {
          ...
          if (failed(alignedLhs)) {
            op->emitError() << "failed to align lhs contribution for Rotom "
                               "elementwise op";
          ...
          FailureOr<Value> alignedRhs = alignDomainPointToOutput(
              ...
              domain, outputLayout, b);
          if (failed(alignedRhs)) {
            op->emitError() << "failed to align rhs contribution for Rotom "
                               "elementwise op";
            loweringResult = failure();
            return;
          }
          ...
          Operation* sum = makeAppropriatelyTypedAddOp(
              b, op->getLoc(), acc, pointValue->getResult(0));
          sum->setAttr(kLayoutAttrName, outputLayout);
          setMaterializedAttr(sum);
          acc = sum->getResult(0);
        });

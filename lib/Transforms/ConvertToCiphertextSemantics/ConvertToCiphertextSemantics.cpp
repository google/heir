...
class ConvertTensorExtractSlice
 ...
 public:
  ...
  LogicalResult secretSourceSecretResult(
    ...
    // A remap's input and output types will stay the same, i.e. the larger
    // shape that contains all of the source ciphertexts. This means we must
    // extract out the result slice ciphertexts from the remap's output.

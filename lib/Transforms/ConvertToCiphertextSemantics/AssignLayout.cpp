...
static FailureOr<Value> implementAssignLayoutStep(
    Value input, LayoutAttr layout, ImplicitLocOpBuilder& builder,
    const std::function<void(Operation*)>& createdOpCallback, bool isLast,
    int64_t ciphertextSize, ArrayRef<int64_t> domainSchedule = {}) {
  ...
  if (isDenseLayout(rel, targetType)) {
    ...
    // ciphertext tensor.
    LLVM_DEBUG(llvm::dbgs() << "Detected dense layout, splatting input\n");
    if (!dataSemanticType) {
  ...
  if (matchPattern(input, m_Constant(&constantAttr)) && dataSemanticType &&
    ...
    LLVM_DEBUG(llvm::dbgs() << "gathered packed values\n");
...
FailureOr<Value> implementAssignLayout(
  ...
  if (auto arrayAttr = dyn_cast<ArrayAttr>(layout)) {
    ...
    for (size_t i = 0; i < arrayAttr.size(); ++i) {
      ...
      auto result =
          implementAssignLayoutStep(currentInput, layoutAttr, builder,
                                    createdOpCallback, isLast, ciphertextSize);
      if (failed(result)) {
        return failure();
      }
  ...
  if (LayoutAttr layoutAttr = dyn_cast<LayoutAttr>(layout)) {
    return implementAssignLayoutStep(input, layoutAttr, builder,
                                     createdOpCallback, /*isLast=*/true,
                                     ciphertextSize, domainSchedule);

...
LogicalResult convertOpOperands(secret::GenericOp op, func::FuncOp func,
  ...
  for (OpOperand &opOperand : op->getOpOperands()) {
    ...
    if (!mlir::isa<secret::SecretType>(opOperand.get().getType())) {
      if (!mlir::isa<IntegerType>(opOperand.get().getType())) {
...
void YosysOptimizer::runOnOperation() {
  ...
  if (useSubmodules) {
    ...
    // If we extracting submodules, ensure that the merged generics do not have
    // index args. Constant accessors have already been absorbed into the
    // generic body. Index types are currently not supported by this pass will
    // return a failure when converting the operand types. Index arguments
    // usually appear as affine indices from loop arguments when the generic is
    // distributed through an affine for loop. In this case, use-submodules
    // should be false. This could also be avoided if we avoided merging
    // generics, but would come with the downside of not being able to globally
    // optimize the program.
    auto checkInputs = op->walk([&](secret::GenericOp op) {
      if (llvm::any_of(op.getInputs(),
                       [](Value t) { return isa<IndexType>(t.getType()); })) {
        op->emitOpError() << "generic incompatible with submodule extraction "
                             "due to secret indices, "
                             "did you mean to use use-submodules=false?";
        return WalkResult::interrupt();
      }
      return WalkResult::advance();
    });

    if (checkInputs.wasInterrupted()) {
      signalPassFailure();
      return;
    }
  }

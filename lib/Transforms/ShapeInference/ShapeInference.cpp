          ...
          funcOp.getBody().walk<WalkOrder::PreOrder>([&](Operation *op) {
            ...
            if (auto regionBranchOp = dyn_cast<RegionBranchOpInterface>(op)) {
              ...
              for (auto &successor : successors) {
                ...
                LLVM_DEBUG(llvm::dbgs()
                           << "ShapeInference:\t\t\tfound region successor: "
                ...
                for (auto [input, operand] :
                     llvm::zip(successorInputs, succesorOperands)) {
                  LLVM_DEBUG(
                      llvm::dbgs()
              ...
              for (auto successor : successors) {
                ...
                if (successor.isParent()) {
                  ...
                  for (auto [operand, result] :
                       llvm::zip(succesorOperands, parent->getResults())) {
                    LLVM_DEBUG(llvm::dbgs()
                               << "ShapeInference:\t\t\tupdating parent result "
                               << result << " with type of successor operand "
                               << operand << "\n");
                  ...
                  LLVM_DEBUG(llvm::dbgs()
                             ...
                             << successor.getSuccessor()->getLoc() << "\n");
                  for (auto [operand, input] : llvm::zip(
                           succesorOperands, successor.getSuccessorInputs())) {
                    LLVM_DEBUG(llvm::dbgs()
                               << "ShapeInference:\t\t\t"

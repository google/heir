...
struct FuseConv2DPooling : public OpRewritePattern<linalg::PoolingNchwSumOp> {
 public:
  ...
  LogicalResult matchAndRewrite(linalg::PoolingNchwSumOp poolOp,
    ...
    // Apply pooling to the (usually cleartext) conv initializer
    Value pooledInit =
        linalg::PoolingNchwSumOp::create(
            rewriter, poolOp.getLoc(), poolOutputType,
            ValueRange{convOp.getOutputs()[0], poolOp.getInputs()[1]},
            ValueRange{poolOp.getOutputs()[0]}, poolOp.getStrides(),
            poolOp.getDilations())
            .getResult(0);

    Value initForConv = pooledInit;
    if (foldDivision) {
      IRMapping mapping;
      mapping.map(poolOp.getResult(0), pooledInit);
      Operation* clonedOp = rewriter.clone(*divOrMulOp, mapping);
      initForConv = clonedOp->getResult(0);
    }
    ...
    Value fusedConv =
        linalg::Conv2DNchwFchwOp::create(
            ...
            ValueRange{initForConv}, effStridesAttr, effDilationsAttr)
            .getResult(0);

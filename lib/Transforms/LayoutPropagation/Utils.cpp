...
LayoutAttr convertLayoutForReduce(LayoutAttr inputLayout,
  ...
  // along dimension 2:
  // Directly constructing the cyclic relation for the remaining dimensions
  ...
  if (slotUb.has_value() && numDomainVars == 3 &&
    ...
    if (shape.size() == 3) {
      ...
      if (isRelationCyclic(tensorType, numSlots, rel)) {
        ...
        return LayoutAttr::getFromIntegerRelation(
            context, getCyclicLayoutRelation(reducedType, numSlots));
      }
    }
  }
  ...
  if (slotUb.has_value() && numDomainVars == 2 &&
    ...
    if (shape.size() == 2) {
      RankedTensorType matrixType =
          RankedTensorType::get(shape, Float32Type::get(context));
      if (isRelationCyclic(matrixType, numSlots, rel)) {
        presburger::IntegerRelation cyclicRel =
            getCyclicLayoutRelation(matrixType, numSlots);
        cyclicRel.projectOut(1, 1);

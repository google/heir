...
struct PopulateScale : impl::PopulateScaleBase<PopulateScale> {
  ...
  void runOnOperation() override {
    auto bgvSchemeParam = mlir::dyn_cast<bgv::SchemeParamAttr>(
        getOperation()->getAttr(bgv::BGVDialect::kSchemeParamAttrName));
    auto Q = bgvSchemeParam.getQ();
    ...
    auto t = bgvSchemeParam.getPlaintextModulus();
    ...
    // adjust_scale[i] = scale[i - 1] * q[i] mod t
    // which means adjust_scale[i] * q[i]^-1 = scale[i - 1] mod t
    std::vector<int64_t> adjustScale(L + 1, 0);
    adjustScale[0] = 1;
    for (auto i = 1; i <= L; i++) {
      auto q = qi[i] % t;
      auto newScale = (adjustScale[i - 1] * q) % t;

...
struct ConvertKeySwitchInner
  ...
  LogicalResult matchAndRewrite(
    ...
    int64_t partSize = 7;  // FIXME: how to select this?
    int64_t numPieces = coefficientTensorType.getShape()[0] / partSize;
    int64_t sliceSize = partSize * numPieces;
    ...
    SmallVector<int64_t> partitionShape = {numPieces, partSize};
    Value reshaped = tensor::ReshapeOp::create(
        b,
        RankedTensorType::get(partitionShape,
                              coefficientTensorType.getElementType()),
        extracted);

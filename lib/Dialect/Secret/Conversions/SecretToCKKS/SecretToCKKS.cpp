...
struct LinearTransformOpConversion
  ...
  LogicalResult matchAndRewrite(
    ...
    llvm::errs() << "LinearTransformOpConversion: ringDim=" << ringDim
                 << ", numCols=" << numCols << ", actualSlots=" << actualSlots
                 << "\n";

    DenseElementsAttr newDiagonalsAttr;
    if (numCols == actualSlots) {
      llvm::errs() << "LinearTransformOpConversion: No padding needed\n";
      newDiagonalsAttr = diagonalsAttr;
    } else {
      llvm::errs() << "LinearTransformOpConversion: Padding from " << numCols
                   << " to " << actualSlots << "\n";
      if (numCols > actualSlots) {
    ...
    llvm::errs() << "LinearTransformOpConversion: newDiagonalsAttr type="
                 << newDiagonalsAttr.getType() << "\n";
    ...
    for (auto attr : dedupedAttrsToPreserve) {
      llvm::errs() << "LinearTransformOpConversion: dedupedAttr: "
                   << attr.getName().getValue() << " = " << attr.getValue()
                   << "\n";
    }

    auto newLtOp = rewriter.create<kernel::LinearTransformOp>(
        ltOp.getLoc(), resultTypes, inputs, dedupedAttrsToPreserve);

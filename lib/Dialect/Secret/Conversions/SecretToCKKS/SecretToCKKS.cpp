...
struct SecretToCKKS : public impl::SecretToCKKSBase<SecretToCKKS> {
  ...
  void runOnOperation() override {
    ...
    auto schemeParam = ckks::SchemeParam::getConcreteSchemeParam(45, logPrimes);
    LLVM_DEBUG(llvm::dbgs() << "Concrete Scheme Param:\n"
                            << schemeParam << "\n");
    // TODO: also annotate ckks::SchemeParamAttr to ModuleOp here
    // not doing now for lattigo not having a pass to clean it up.

...
// Returns true if `value` carries ciphertext data (a secret type). Work on
// a non-secret value is host-side plaintext preparation: real, but orders of
// magnitude cheaper than the same op on a ciphertext, and often folded into
// the encoded constants outright.
bool isCiphertext(Value value);

...
struct Candidate {
  ...
  // True when the op has a Rotom elementwise lowering that can align its
  // operands via conversion (ranked like a named kernel).
  bool hasRotomKernel = false;
  int64_t localCost = 0;
...
// when it carries a Rotom kernel, then by a deterministic structural tie key.

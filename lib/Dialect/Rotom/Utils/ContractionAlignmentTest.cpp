...
// 4x4 matmul at n=16, both operands row-major (iteration dims: i=0, j=1,
// k=2; lhs holds (i,k), rhs holds (k,j)). The four host x region variants
// dedup to three plans: rhs-hosted/ct coincides with lhs-hosted/slot.
TEST_F(ContractionAlignmentTest, RowMajorPairEnumeratesThreeDedupedPlans) {
  LayoutAttr lhs = layout({dim(0, 4), dim(2, 4)}, 16);
  LayoutAttr rhs = layout({dim(2, 4), dim(1, 4)}, 16);
...
TEST_F(ContractionAlignmentTest, LhsHostedSlotPlan) {
  ...
  EXPECT_EQ(plan->expandedLhs, layout({dim(0, 4), dim(2, 4), repl(4)}, 16));
  EXPECT_EQ(plan->expandedRhs, layout({repl(4), dim(2, 4), dim(1, 4)}, 16));
...
TEST_F(ContractionAlignmentTest, LhsHostedCtPlan) {
  ...
  ASSERT_NE(plan, nullptr);
  EXPECT_EQ(plan->expandedLhs, layout({repl(4), dim(0, 4), dim(2, 4)}, 16));
  EXPECT_EQ(plan->expandedRhs, layout({dim(1, 4), repl(4), dim(2, 4)}, 16));
...
TEST_F(ContractionAlignmentTest, RhsHostedSlotPlanSumsAcrossCiphertexts) {
  ...
  ASSERT_NE(plan, nullptr);
  EXPECT_EQ(plan->expandedLhs, layout({dim(2, 4), repl(4), dim(0, 4)}, 16));
  EXPECT_EQ(plan->expandedRhs, layout({dim(2, 4), dim(1, 4), repl(4)}, 16));
...
TEST_F(ContractionAlignmentTest, UnitFreeDimSkipsInsertion) {
  LayoutAttr lhs = layout({dim(0, 4), dim(2, 4)}, 16);
  LayoutAttr rhs = layout({dim(2, 4)}, 16);
  SmallVector<MatmulPlan> plans = rotom::enumerateMatmulPlans(lhs, rhs);
  ...
  EXPECT_EQ(plan->expandedLhs, plan->computeLayout);
  EXPECT_EQ(plan->expandedRhs, layout({repl(4), dim(2, 4)}, 16));
...
TEST_F(ContractionAlignmentTest, NonPowerOfTwoFreeExtentStaysInCtRegion) {
  LayoutAttr lhs = layout({dim(0, 4), dim(2, 4)}, 16);
  LayoutAttr rhs = layout({dim(2, 4), dim(1, 3)}, 16);
  SmallVector<MatmulPlan> plans = rotom::enumerateMatmulPlans(lhs, rhs);
  EXPECT_EQ(plans.size(), 3u);
  for (const MatmulPlan& plan : plans) {
    EXPECT_EQ(plan.lhsFillRotations, 0);
...
TEST_F(ContractionAlignmentTest, MixedRadixHostCarriesThroughAndDropsCtK) {
  LayoutAttr lhs =
      layout({dim(0, 2, /*stride=*/2), dim(2, 4), dim(0, 2, /*stride=*/1)}, 16);
  LayoutAttr rhs = layout({dim(2, 4), dim(1, 4)}, 16);
  SmallVector<MatmulPlan> plans = rotom::enumerateMatmulPlans(lhs, rhs);
...
TEST_F(ContractionAlignmentTest, MismatchedNReturnsNoPlans) {
  LayoutAttr lhs = layout({dim(0, 4), dim(2, 4)}, 16);
  LayoutAttr rhs = layout({dim(2, 4), dim(1, 4)}, 32);

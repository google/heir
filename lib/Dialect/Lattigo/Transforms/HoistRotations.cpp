...
struct LattigoHoistRotations
  ...
  void runOnOperation() override {
    getOperation().walk([&](func::FuncOp op) -> WalkResult {
      // BGV
      auto bgvResult = getArgOfType<lattigo::BGVEvaluatorType>(op);
      if (succeeded(bgvResult)) {
        Value evaluator = bgvResult.value();
        processFunc<lattigo::BGVRotateColumnsNewOp, lattigo::RotateHoistedNewOp,
                    lattigo::LookupRotatedOp>(op, evaluator);
      ...
      if (succeeded(ckksResult)) {
        ...
        processFunc<lattigo::CKKSRotateNewOp, lattigo::CKKSRotateHoistedNewOp,
                    lattigo::CKKSLookupRotatedOp>(op, evaluator);
        return WalkResult::advance();
      }
 ...
 private:
  ...
  void processFunc(func::FuncOp funcOp, Value evaluator) {
    ...
    for (auto const& [ciphertext, rots] : ciphertextToDistinctRotations) {
      ...
      auto hoistedRotateOp = builder.create<HoistedRotateOp>(
          ciphertext.getLoc(),
      ...
      for (RotateOp op : ciphertextToRotateOps[ciphertext]) {
        ...
        auto lookupOp = builder.create<LookupOp>(op.getLoc(), op.getType(),
                                                 hoistedRotateOp.getResult(),
                                                 op.getOffset());

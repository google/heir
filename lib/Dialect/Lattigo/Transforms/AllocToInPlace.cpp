...
struct ConvertRotateOp : public OpRewritePattern<RotateOp> {
  ...
  LogicalResult matchAndRewrite(RotateOp op,
    ...
    // InPlaceOp has the form: output = InPlaceOp(evaluator, input, inplace, shift?, offset?)
    // where inplace is the actual output but for SSA form we need to
    // return a new value
    // Handle both shift SSA value and offset attribute

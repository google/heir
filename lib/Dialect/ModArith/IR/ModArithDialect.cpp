...
LogicalResult ConstantOp::verify() {
  ...
  // Note: comparison will crash if the bitwidths aren't the same
  if (valueBW > modBW ||
      (valueBW == modBW && !getValue().ult(getType().getModulus().getValue())))
    return emitOpError("Constant value must be smaller than modulus.");

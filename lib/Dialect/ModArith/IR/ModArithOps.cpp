...
LogicalResult ConstantOp::verify() {
  ...
  auto denseAttr = dyn_cast<ElementsAttr>(getValue());
  ...
  if (shapedType == nullptr && denseAttr != nullptr)
    return emitOpError() << "found dense attr " << denseAttr
                         << " but scalar type " << getType();
  if (shapedType != nullptr && denseAttr == nullptr)
    return emitOpError() << "found non-dense attr " << denseAttr
                         << " but shaped type " << shapedType;

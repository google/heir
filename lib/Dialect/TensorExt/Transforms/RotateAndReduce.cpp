...
struct RotateAndReduce : impl::RotateAndReduceBase<RotateAndReduce> {
  ...
  void tryReplaceRotations(ArithOp op,
    ...
    auto rotateAndReduceOp = b.create<tensor_ext::RotateAndReduceOp>(
        tensor.getType(),               // output type same as input
        tensor,                         // input tensor
        Value{},                        // no plaintexts for accumulation case
        b.getIndexAttr(1),              // period = 1
        b.getIndexAttr(tensorShape[0])  // steps = tensor size
    );

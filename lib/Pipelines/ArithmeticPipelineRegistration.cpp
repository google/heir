...
void mlirToRLWEPipeline(OpPassManager &pm,
  ...
  // which is required for secret-to-secret lowering
  switch (scheme) {
    case RLWEScheme::bgvScheme: {
      auto secretWithMgmtBGVOptions = SecretWithMgmtBGVOptions{};
      secretWithMgmtBGVOptions.includeFirstMul =
          options.modulusSwitchBeforeFirstMul;
      pm.addPass(createSecretWithMgmtBGV(secretWithMgmtBGVOptions));
      break;
    }
    default:
      break;

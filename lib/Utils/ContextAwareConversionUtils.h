...
class SecretGenericOpRotateConversion
 ...
 public:
  ...
  FailureOr<Operation*> matchAndRewriteInner(
      ...
      ArrayRef<NamedAttribute> attributes,
      ContextAwareConversionPatternRewriter& rewriter) const override {
    // Pass the dynamic_shift as an SSA value. If it happens to be a constant,
    // the backend can optimize it to an attribute later if needed.
    auto dynamicShift = inputs[1];
    auto newOp = T::create(rewriter, op.getLoc(), outputTypes, inputs[0],
                           dynamicShift, /*static_shift=*/nullptr);

...
LogicalResult ContextAwareConversionPatternRewriterImpl::remapValues(
  ...
  for (const auto &it : llvm::enumerate(values)) {
    ...
    if (failed(currentContextAwareTypeConverter->convertType(origType, operand,
                                                             legalTypes))) {
...
struct FunctionOpInterfaceSignatureConversion
  ...
  FunctionOpInterfaceSignatureConversion(
      StringRef functionLikeOpName, MLIRContext *ctx,
      const ContextAwareTypeConverter &converter)
      : ContextAwareConversionPattern(converter, functionLikeOpName,
                                      /*benefit=*/1, ctx) {}
...
struct AnyFunctionOpInterfaceSignatureConversion
    : public OpInterfaceContextAwareConversionPattern<FunctionOpInterface> {
  using OpInterfaceContextAwareConversionPattern::
      OpInterfaceContextAwareConversionPattern;

  LogicalResult matchAndRewrite(
      FunctionOpInterface funcOp, ArrayRef<Value> /*operands*/,
      ContextAwareConversionPatternRewriter &rewriter) const override {
    return convertFuncOpTypes(funcOp, *typeConverter, rewriter);
  }
};
...
FailureOr<Operation *> convertOpResultTypes(
  ...
  if (failed(converter.convertTypes(op->getResultTypes(), op->getResults(),
                                    newResultTypes)))
...
LogicalResult applyPartialConversion(Operation *op,
                                     const ConversionTarget &target,
                                     const FrozenRewritePatternSet &patterns,
                                     ConversionConfig config) {

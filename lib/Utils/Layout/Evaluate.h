...
// This unpacks a layout relation (ct, slot) on a given n-D vector.
//
// This is intended to be used for test vectors, since the performance is not
// optimized.
template <typename T>
void unpackLayout(
    ...
    std::function<void(const std::vector<int64_t>&, T)> setValueFn) {
  // Get all points in the relation.
  PointPairCollector collector(relation.getNumDomainVars(), /*rangeDims=*/2);
  enumeratePoints(relation, collector);

  for (const auto& pointPair : collector.points) {
    const std::vector<int64_t>& domainPoint = pointPair.first;
    const std::vector<int64_t>& rangePoint = pointPair.second;
    setValueFn(domainPoint, packed[rangePoint[0]][rangePoint[1]]);
  }
}

// Unpack a packed (ct x slots) representation back to a 1-D vector with the
// given originalShape {n}.
template <typename T>
std::vector<T> unpackLayoutToVector(const presburger::IntegerRelation& relation,
                                    const std::vector<std::vector<T>>& packed,
                                    ArrayRef<int64_t> originalShape) {
  assert(originalShape.size() == 1 && "originalShape must be 1-D");
  std::vector<T> result(originalShape[0], 0);

  unpackLayout<T>(relation, packed,
                  [&](const std::vector<int64_t>& domainPoint, T value) {
                    result[domainPoint[0]] = value;
                  });
  return result;
}

// Unpack a packed (ct x slots) representation back to a 2-D matrix with the
// given originalShape {m, n}.
template <typename T>
std::vector<std::vector<T>> unpackLayoutToMatrix(
    const presburger::IntegerRelation& relation,
    const std::vector<std::vector<T>>& packed,
    ArrayRef<int64_t> originalShape) {
  assert(originalShape.size() == 2 && "originalShape must be 2-D");
  std::vector<std::vector<T>> result(originalShape[0],
                                     std::vector<T>(originalShape[1], 0));

  unpackLayout<T>(relation, packed,
                  [&](const std::vector<int64_t>& domainPoint, T value) {
                    result[domainPoint[0]][domainPoint[1]] = value;
                  });
  return result;
}
...
template <typename T>
std::vector<std::vector<std::vector<std::vector<T>>>> unpackLayoutTo4DTensor(
    const presburger::IntegerRelation& relation,
    const std::vector<std::vector<T>>& packed,
    ArrayRef<int64_t> originalShape) {
  assert(originalShape.size() == 4 && "originalShape must be 4-D");
  std::vector<std::vector<std::vector<std::vector<T>>>> result(
      originalShape[0],
      std::vector<std::vector<std::vector<T>>>(
          originalShape[1],
          std::vector<std::vector<T>>(originalShape[2],
                                      std::vector<T>(originalShape[3], 0))));

  unpackLayout<T>(
      relation, packed, [&](const std::vector<int64_t>& domainPoint, T value) {
        result[domainPoint[0]][domainPoint[1]][domainPoint[2]][domainPoint[3]] =
            value;
      });
  return result;
}

...
presburger::IntegerRelation get2dConvFilterRelation(RankedTensorType filterType,
  ...
  // These are the indices that represent the position of the filter as it
  ...
  addBounds(result, dataRow, -padding,
            -padding + (2 * padding + dataRowSize - filterRowSize));
  addBounds(result, dataCol, -padding,
            -padding + (2 * padding + dataColSize - filterColSize));

  // Add constraints for when the resulting filter index is in range.
  // 0 <= filterRow + dataRow < filterRowSize
  addConstraint(result, {{filterRow, 1}, {dataRow, 1}}, /*equality=*/false);
  addConstraint(
      result, {{constCoeff, filterRowSize - 1}, {filterRow, -1}, {dataRow, -1}},
      /*equality=*/false);

  // 0 <= filterCol + dataCol < filterColSize
  addConstraint(result, {{filterCol, 1}, {dataCol, 1}}, /*equality=*/false);
  addConstraint(
      result, {{constCoeff, filterColSize - 1}, {filterCol, -1}, {dataCol, -1}},
      /*equality=*/false);
  ...
  //
  // mr = (idr + P)*fr + (idc + P) = P + P*fr + idr * fr + idc
  // mc = -(P + fc * P) + mr + (ifc) + fr*(ifr)
  addConstraint(result,
                {{matRow, -1},
                 {constCoeff, dataRowSize * padding + padding},
                 {dataRow, dataRowSize},
                 {dataCol, 1}},
                /*equality=*/true);
  addConstraint(result,

...
static __isl_give isl_union_map* convertRelationToUnionMap(
  ...
  // It would be helpful to set dim names, but we don't have access to them
  // in the relation... Maybe add it?
  //
  // for (int i = 0; i < rel.getNumVars(); i++) {
  //   isl_dim_type dimKind;
  //   unsigned offset = 0;
  //   if (rel.getVarKindAt(i) == VarKind::Domain) {
  //     dimKind = isl_dim_in;
  //     offset = rel.getVarKindOffset(VarKind::Domain);
  //   } else if (rel.getVarKindAt(i) == VarKind::Range) {
  //     dimKind = isl_dim_out;
  //     offset = rel.getVarKindOffset(VarKind::Range);
  //   } else if (rel.getVarKindAt(i) == VarKind::Local) {
  //     dimKind = isl_dim_div;
  //     offset = rel.getVarKindOffset(VarKind::Local);
  //   }
  //   char *name = "???";
  //   space = isl_space_set_dim_name(space, dimKind, offset, name);
  // }

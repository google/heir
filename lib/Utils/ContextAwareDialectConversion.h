...
/// OpInterfaceContextAwareConversionPattern is a wrapper around
/// ContextAwareConversionPattern that allows for matching and rewriting against
/// an instance of an OpInterface class as opposed to a raw Operation.
template <typename SourceOp>
class OpInterfaceContextAwareConversionPattern
    : public ContextAwareConversionPattern {
 public:
  OpInterfaceContextAwareConversionPattern(MLIRContext *context,
                                           PatternBenefit benefit = 1)
      : ContextAwareConversionPattern(Pattern::MatchInterfaceOpTypeTag(),
...
/// OpTraitContextAwareConversionPattern is a wrapper around
/// ContextAwareConversionPattern that allows for matching and rewriting against
/// instances of an operation that possess a given trait.
template <template <typename> class TraitType>
class OpTraitContextAwareConversionPattern
    : public ContextAwareConversionPattern {
 public:
  OpTraitContextAwareConversionPattern(MLIRContext *context,
                                       PatternBenefit benefit = 1)
      : ContextAwareConversionPattern(Pattern::MatchTraitOpTypeTag(),
                                      TypeID::get<TraitType>(), benefit,
                                      context) {}
  OpTraitContextAwareConversionPattern(
      const ContextAwareTypeConverter &typeConverter, MLIRContext *context,
      PatternBenefit benefit = 1)
      : ContextAwareConversionPattern(
            typeConverter, Pattern::MatchTraitOpTypeTag(),
...
FailureOr<Operation *> convertOpResultTypes(
...
void populateFunctionOpInterfaceTypeContextAwareConversionPattern(
    StringRef functionLikeOpName, RewritePatternSet &patterns,
    const ContextAwareTypeConverter &converter);

template <typename FuncOpT>
void populateFunctionOpInterfaceTypeContextAwareConversionPattern(
    RewritePatternSet &patterns, const ContextAwareTypeConverter &converter) {
  populateFunctionOpInterfaceTypeContextAwareConversionPattern(
      FuncOpT::getOperationName(), patterns, converter);
}

void populateAnyFunctionOpInterfaceTypeContextAwareConversionPattern(
    RewritePatternSet &patterns, const ContextAwareTypeConverter &converter);

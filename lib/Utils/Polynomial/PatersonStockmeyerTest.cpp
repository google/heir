...
TEST(PatersonStockmeyerEvaluationTest, MultiplicativeDepthAdvantage) {
  // High-degree polynomial where P-S should have better depth than Horner
  // p(x) = 1 + x + x^2 + ... + x^15 (degree 15)
  std::map<int64_t, double> coefficients;
  for (int i = 0; i <= 15; ++i) {
    coefficients[i] = 1.0;
  }
  ...
  // For degree 15, P-S with k=sqrt(15)≈4 should have depth around 6-8
  // whereas Horner would have depth 15
  EXPECT_LT(depth, 10);  // Should be significantly less than 15
  EXPECT_GT(depth, 3);   // But more than trivial depth
}
...
TEST(PatersonStockmeyerEvaluationTest, NumericalAccuracy) {
  ...
  // Check roots
  EXPECT_NEAR(evalPatersonStockmeyerPolynomial(1.0, coefficients), 0.0, 1e-10);
  EXPECT_NEAR(evalPatersonStockmeyerPolynomial(2.0, coefficients), 0.0, 1e-10);
  EXPECT_NEAR(evalPatersonStockmeyerPolynomial(3.0, coefficients), 0.0, 1e-10);

  // Check other values
  EXPECT_NEAR(evalPatersonStockmeyerPolynomial(0.0, coefficients), -6.0, 1e-10);
  EXPECT_NEAR(evalPatersonStockmeyerPolynomial(4.0, coefficients), 6.0, 1e-10);
}

TEST(PatersonStockmeyerEvaluationTest, DegreeOneOptimization) {
  // For degree 1, should choose k=1 and work like standard evaluation
  std::map<int64_t, double> coefficients = {{0, 3.0}, {1, 2.0}};
...
TEST(PatersonStockmeyerEvaluationTest, PowerPrecomputationVerification) {
  // Test that power precomputation works correctly for medium-degree polynomial
  // p(x) = x^8 (only highest degree term)

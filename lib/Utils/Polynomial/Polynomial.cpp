...
FloatPolynomial ChebyshevPolynomial::toMonomialBasis() const {
...
ChebyshevPolynomial ChebyshevPolynomial::compose(
  ...
  // from "Chebyshev Polynomials in Numerical Analysis" by Fox and Parker,
  // which is derived from Clenshaw's algorithm for evaluating a Chebyshev
  // series.
  //
  // Let H(x) = sum c_i T_i(x). We want to compute H(y).
  // We use the recurrence d_k = c_k + 2*y*d_{k+1} - d_{k+2}, with
  // d_{n+1}=d_{n+2}=0. The result is H(y) = (d_0 - d_2)/2.
  //
  ...
  // The recurrence that follows is only valid for the non-constant terms of
  // the Chebyshev series. The constant term c_0 is handled separately.
  APFloat c0 = c[0];
  c[0] = APFloat(0.0);
  ...
  // The final coefficients are (d_0 - d_2)/2. This gives the composition
  // for the series with the constant term removed.
  ...
  resultCoeffs[0] += c0.convertToDouble();

  FloatPolynomial result = toMonomialBasis().compose(other);

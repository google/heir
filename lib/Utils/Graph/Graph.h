 ...
 public:
  ...
  std::unordered_map<V, int> color(const UndirectedGraph<V>& graph) {
    ...
    while (!queue.empty()) {
      ...
      // debug print
      std::cout << "vertex: " << current.vertex
                << " saturation: " << current.saturationDegree
                << " uncolored: " << current.uncoloredDegree << std::endl;
 ...
 private:
  ...
  void updateSaturationDegree(const UndirectedGraph<V>& graph,
    ...
    VertexInfo info{vertex, static_cast<int>(neighborColors[vertex].size()),
                    uncolored};
    std::cout << "UPDATING: vertex: " << vertex
              << " saturation: " << info.saturationDegree
              << " uncolored: " << info.uncoloredDegree << std::endl;

...
void NoisePropagationAnalysis::visitOperation(
  ...
  // Ops with argument-independent noise propagation can work with unbounded
  // and uninitialized noise arguments, but others cannot. If we encounter
  // a situation where an argument-noise-dependent op processes unknown args,
  // we set a pessimistic unbounded variance on the results.
  if (!noisePropagationOp.hasArgumentIndependentResultNoise() &&
      llvm::any_of(argRanges, [](const Variance variance) {
        return !variance.isKnown();
      })) {
    ...
    for (auto result : op->getResults()) {
      ...
      ChangeResult changed = lattice->join(Variance::unbounded());
  ...
  auto joinCallback = [&](Value value, const Variance &variance) {
    ...
    ChangeResult changed = lattice->join(variance);

    // If the result is yielded, then it is assumed to be in a loop yield. Then
    // the best we can do is check to see if the op producing this value has
    // argument-independent noise. If so, we can propagate that noise.
    // Otherwise, we must assume the worst case scenario of unbounded noise.
    ...
    if (isYieldedResult && oldRange.isKnown() &&
      ...
      changed |= lattice->join(Variance::unbounded());
    }
    propagateIfChanged(lattice, changed);
  };

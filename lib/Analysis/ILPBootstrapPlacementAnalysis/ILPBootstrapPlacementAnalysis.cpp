...
LogicalResult ILPBootstrapPlacementAnalysis::solve() {
  ...
  const int BIG_M = bootstrapWaterline * 2;
  ...
  for (BlockArgument arg : body->getArguments()) {
    ...
    auto levelVar = model.AddContinuousVariable(0, bootstrapWaterline, ss.str());
  ...
  for (Operation& op : body->getOperations()) {
    ...
    if (llvm::none_of(op.getResults(),
        [&](OpResult result) { return isSecret(result, solver); })) {
      continue;
    }
    ...
    auto inputLevelVar =
        model.AddContinuousVariable(0, bootstrapWaterline, "inputLevel" + opName);
...
llvm::SmallVector<Value, 32> ILPBootstrapPlacementAnalysis::getValuesToBootstrap() const {

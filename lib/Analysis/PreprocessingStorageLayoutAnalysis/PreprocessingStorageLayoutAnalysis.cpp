...
PreprocessingStorageLayoutAnalysis::PreprocessingStorageLayoutAnalysis(
  ...
  WalkResult storeResult = op->walk([&](Operation* subOp) {
    if (subOp != op && subOp->hasTrait<OpTrait::SymbolTable>()) {
      return WalkResult::skip();
    }
    if (auto storeOp = dyn_cast<preprocessing::StoreOp>(subOp)) {
      uint32_t siteId = storeOp.getSiteId();
      if (storesBySite.count(siteId)) {
        storeOp->emitError()
            << "Duplicate StoreOp found for site_id: " << siteId;
  ...
  WalkResult loadResult = op->walk([&](Operation* subOp) {
    if (subOp != op && subOp->hasTrait<OpTrait::SymbolTable>()) {
      return WalkResult::skip();
    }
    if (auto loadOp = dyn_cast<preprocessing::LoadOp>(subOp)) {
  ...
  for (uint32_t siteId : siteIds) {
    ...
    siteLayouts[siteId] = {currentOffset, *size};
...
FailureOr<SiteLayout> PreprocessingStorageLayoutAnalysis::getLayout(
  ...
  auto it = siteLayouts.find(siteId);
  if (it == siteLayouts.end()) return failure();
  return it->second;
}

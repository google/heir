...
Value multiplyDiagonalizedMatrixWithVector(
  ...
  //   %extractedSlice = extract_slice %newMatrix[%i, 0] [1,
  //   originalMatrixDimensions[0]] [1, 1] %multiplied = %rotated_vector *
  //   %extractedSlice %sum = %sum + %multiplied %rotated_vector = rotate
  //   %rotated_vector, 1
  // %lastExtracted = extract_slice %newMatrix[maxTilingSize-1, 0]
  // [1, originalMatrixDimensions[0]] [1, 1] %final_sum = %sum +
  // %lastExtracted At this point, we can rotate and sum. return %final_sum
...
struct SecretGenericOpLinalgMatmulConversion
    : public SecretGenericOpConversion<mlir::linalg::MatmulOp> {
 private:
  DataFlowSolver *solver;
  int maxTilingSize;

 public:
  SecretGenericOpLinalgMatmulConversion(const TypeConverter &typeConverter,
                                        DataFlowSolver *solver,
                                        mlir::MLIRContext *context,
                                        int maxTilingSize)
      : SecretGenericOpConversion<mlir::linalg::MatmulOp>(typeConverter,
                                                          context),
        solver(solver),
        maxTilingSize(maxTilingSize) {}
  using SecretGenericOpConversion<
      mlir::linalg::MatmulOp>::SecretGenericOpConversion;

  LogicalResult matchAndRewriteInner(
      secret::GenericOp genericOp, TypeRange genericOpOutputTypes,
      ValueRange genericOpInputTypes, ArrayRef<NamedAttribute> attributes,
    ...
    // Define local function pointers or lambdas that refer to your functions
    ...
    auto newGeneric = rewriter.create<secret::GenericOp>(
        genericOp.getLoc(), genericOpInputTypes, genericOpOutputTypes,
...
struct LinalgToTensorExt
  ...
  void runOnOperation() override {
    ...
    target.addDynamicallyLegalOp<secret::GenericOp>([&](secret::GenericOp op) {
      auto matmulOp = dyn_cast<mlir::linalg::MatmulOp>(
      ...
      bool isLeftOperandSecret = isSecret(matmulOp.getInputs()[0]);
      bool isRightOperandSecret = isSecret(matmulOp.getInputs()[1]);

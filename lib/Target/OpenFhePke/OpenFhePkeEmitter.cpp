...
static void printFloatValue(const APFloat &apValue, raw_ostream &os) {
  ...
  if (!APFloat(apValue.getSemantics(), strValue).bitwiseIsEqual(apValue)) {
    ...
    assert(strValue.str().contains('.') && "expected decimal notation");
  }
...
FailureOr<std::string> printOneDimDenseElementsAttr(DenseElementsAttr attr) {
  ...
  auto printEltFn = [&](unsigned index) {
    if (elementType.isIntOrIndex()) {
      ...
      printFloatValue(*(valueIt + index), ss);
    }
  };
  if (attr.isSplat()) {
    printEltFn(0);
...
LogicalResult OpenFhePkeEmitter::printOperation(affine::AffineForOp op) {
  if (!op.getConstantLowerBound() || !op.getConstantUpperBound()) {
    return failure();
  }

  // Clone the loop's results to use as initial iter args. We must use non-const
  // types so that the loop body can modify them.
  for (auto i = 0; i < op.getNumRegionIterArgs(); ++i) {
    if (failed(emitTypedAssignPrefix(op.getResults()[i], op.getLoc(), false))) {
      return failure();
    }
    os << variableNames->getNameForValue(op.getOperands()[i]) << "->Clone();\n";
...
LogicalResult OpenFhePkeEmitter::printOperation(tensor::ExtractSliceOp op) {
  ...
  os << "std::vector<int64_t> " << resultVarName << "(std::begin("
     << inputVarName << ") + " << flattenStart << ", std::end(" << inputVarName
     << ") + " << flattenEnd << ");\n";
  return success();
}

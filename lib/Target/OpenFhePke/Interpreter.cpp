  ...
  if (auto intAttr = dyn_cast<IntegerAttr>(valueAttr)) {
    ...
    if (elementsAttr.getType().getElementType().isInteger(32) ||
      ...
      env.try_emplace(op.getResult(), std::move(values));
               ...
               elementsAttr.getType().getElementType().isF64()) {
...
TypedCppValue Interpreter::applyBinop(
    ...
    std::function<float(float, float)> floatFunc) {
  if (std::holds_alternative<int>(lhs.value) &&
    ...
    return TypedCppValue(
        floatFunc(std::get<float>(lhs.value), std::get<float>(rhs.value)));
    ...
    for (size_t i = 0; i < lhsVec.size(); ++i)
      resultVec[i] = floatFunc(lhsVec[i], rhsVec[i]);
    return TypedCppValue(resultVec);
  } else {
...
void Interpreter::visit(arith::SelectOp op) {
  ...
  if (std::holds_alternative<bool>(condition.value)) {
    ...
    throw std::runtime_error("Select condition must be bool, int, or float");
  }
...
void Interpreter::visit(arith::ExtFOp op) {
  auto operand = env.at(op.getIn());
  // ExtFOp can extend float precision or convert int to float
  if (std::holds_alternative<float>(operand.value)) {
    ...
    env.try_emplace(op.getResult(),
                    static_cast<float>(std::get<int>(operand.value)));
  } else if (std::holds_alternative<std::vector<int>>(operand.value)) {
    // Convert vector<int> to vector<float>
    const auto& vec = std::get<std::vector<int>>(operand.value);
    std::vector<float> result(vec.begin(), vec.end());
    env.try_emplace(op.getResult(), std::move(result));
  } else if (std::holds_alternative<std::vector<float>>(operand.value)) {
    // Already a vector of floats
    env.try_emplace(op.getResult(),
                    std::get<std::vector<float>>(operand.value));
  } else {
    op->emitError() << "ExtFOp received operand with variant index "
                    << operand.value.index();
    throw std::runtime_error("ExtFOp requires float, int, or vector operand");
  }
}
...
void Interpreter::visit(tensor::FromElementsOp op) {
  ...
  // Check if we have integer or float elements
  if (elements.empty()) {
...
void Interpreter::visit(scf::IfOp op) {
  ...
  if (std::holds_alternative<bool>(condition.value)) {
    ...
    throw std::runtime_error("If condition must be bool, int, or float");
  }

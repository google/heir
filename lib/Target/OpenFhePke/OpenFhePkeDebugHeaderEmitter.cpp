...
LogicalResult translateToOpenFhePkeDebugHeaderEmitter(Operation* op, 
    ...
    OpenFhePkeDebugHeaderEmitter emitter(os, &variableNames, importType);
...
LogicalResult OpenFhePkeDebugHeaderEmitter::printOperation(func::FuncOp funcOp) {
  ...
  if ( (!isDebugPort(funcOp.getName())) ||
   (!funcOp.isDeclaration()) || (isEmitted)
    ) {
    return success();
  }
...
LogicalResult  OpenFhePkeDebugHeaderEmitter::emitType(Type type, Location loc) {

...
> **NOTE:** The cost of a kernel is calculated using symbolic execution of kernel
> DAGs, as described in the kernel cost model implementation (see
> `lib/Transforms/LayoutOptimization/LayoutOptimization.cpp:390-554`). The cost
> accounts for rotation operations, which dominate FHE latency. Currently, only
> rotation costs are modeled; multiplication depth is not yet included.

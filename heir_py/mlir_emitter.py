...
class TextualMlirEmitter:
    ...
    def emit(self):
        ...
        # FIXME: use inferred or explicit types for args
        args_str = ", ".join([f"%{name}: i64" for name in self.ssa_ir.arg_names])

        # FIXME: get inferred or explicit return types
...
"""
    ...
    def emit_assign(self, assign):
        match assign.value:
            ...
            case ir.Expr(op="binop"):
                ...
                # FIXME: replace i64 with inferred type
            ...
            case ir.Const():
                name = self.get_or_create_name(assign.target)
                # FIXME: fix type (somehow the pretty printer on assign.value knows it's
                # an int???)
    ...
    def emit_return(self, ret):
        ...
        # FIXME: replace i64 with inferred or explicit return type

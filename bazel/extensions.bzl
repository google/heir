...
def _llvm_deps_impl(_):
    ...
    maybe(
        http_archive,
        name = "llvm_zlib",
        # HEIR-local BUILD file: a copy of the upstream
        # @llvm-raw//utils/bazel/third_party_build:zlib-ng.BUILD with
        # `includes = ["."]` added so that LLVM's `#include <zlib.h>` (angled)
        # resolves under HEIR's hermetic clang-22 toolchain, which provides no
        # system include dirs. See bazel/zlib-ng.BUILD for the full rationale.
        build_file = "@heir//bazel:zlib-ng.BUILD",

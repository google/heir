...
func.func @extract_at_secret_index(%arg0: !secret.secret<tensor<32xi16>>, %arg1: !secret.secret<index>) -> !secret.secret<i16> {
    ...
    ^bb0(%arg2: tensor<32xi16>, %arg3: index):
      ...
      // CHECK-NEXT: %[[INNER_FOR:.*]] = affine.for %[[J:.*]] = 0 to 32
      ...
      // CHECK-NEXT: } else {
      // CHECK-NEXT:   scf.yield %[[DUMMY_VAL:.*]] : i16
      %1 = affine.for %i = 0 to 32 iter_args(%sum = %c0_i16) -> (i16) {
        %extracted = tensor.extract %arg2[%arg3] {size = 32} : tensor<32xi16>
        %extracted_0 = tensor.extract %arg2[%i] {size = 32} : tensor<32xi16>

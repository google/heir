...
//
// RUN: heir-opt %s --mlir-to-plaintext-backend=insert-debug-handler-calls=true \
// RUN:   | mlir-translate --mlir-to-llvmir | llc -filetype=obj > %t
// RUN: cc %t %S/libdot_product_8f_debug_test.a %S/../libmemrefCopy.a -o a.out
// RUN: ./a.out | FileCheck %s

// fail it on purpose for dumpping stderr
// CHECK: Test failed

// The --mlir-to-ckks pipeline will automatically rewrite a
// matrix multiplication into a Halevi-Shoup diagonalized matrix multiplication.
// This example is preserved to show a speedup of that pass from a naive
// implementation.

module {
  func.func @matmul(%arg0: tensor<16xf32> {secret.secret}, %arg1: tensor<16xf32> {secret.secret}) -> tensor<16xf32> {
    %cst = arith.constant dense<""> : tensor<16x16xf32>
    %0 = affine.for %arg2 = 0 to 16 iter_args(%arg3 = %arg1) -> (tensor<16xf32>) {
      %1 = affine.for %arg4 = 0 to 16 iter_args(%arg5 = %arg3) -> (tensor<16xf32>) {
        %extracted = tensor.extract %arg0[%arg4] : tensor<16xf32>
        %extracted_0 = tensor.extract %cst[%arg4, %arg2] : tensor<16x16xf32>
        %extracted_1 = tensor.extract %arg5[%arg2] : tensor<16xf32>
        %3 = arith.mulf %extracted, %extracted_0 : f32
        %4 = arith.addf %extracted_1, %3 : f32
        %inserted = tensor.insert %4 into %arg5[%arg2] : tensor<16xf32>
        affine.yield %inserted : tensor<16xf32>
      }
      affine.yield %1 : tensor<16xf32>
    }
    return %0 : tensor<16xf32>
  }
}

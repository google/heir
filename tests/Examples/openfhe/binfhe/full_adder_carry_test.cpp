...
#include "tests/Examples/openfhe/binfhe/full_adder_carry_lib.h"

class BinFHELogicTest_FullAdderCarry : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(BinFHELogicTest_FullAdderCarry, TestFullAdderCarry) {
  using namespace lbcrypto;
  for (int a = 0; a <= 1; a++)
    for (int b = 0; b <= 1; b++)
      for (int cin = 0; cin <= 1; cin++) {
        ...
        auto ct = full_adder_carry(cryptoContext, ct_a, ct_b, ct_cin);
        LWEPlaintext r;

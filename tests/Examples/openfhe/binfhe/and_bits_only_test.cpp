...
#include "tests/Examples/openfhe/binfhe/and_bits_only_lib.h"

class BinFHELogicTest_AND : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(BinFHELogicTest_AND, TestAND) {
  using namespace lbcrypto;
  ...
  for (auto [a, b] : cases) {
    ...
    auto ct = and_bits(cryptoContext, ct_a, ct_b);
    LWEPlaintext r;
    cryptoContext->Decrypt(secretKey, ct, &r, 8);
    EXPECT_EQ(r, (a & b) ? 1 : 0) << "AND(" << a << "," << b << ")";
  }
}

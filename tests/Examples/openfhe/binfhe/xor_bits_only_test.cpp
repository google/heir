...
#include "tests/Examples/openfhe/binfhe/xor_bits_only_lib.h"

class BinFHELogicTest_XOR : public ::testing::Test {
 protected:
  void SetUp() override {
    using namespace lbcrypto;
    auto cc = BinFHEContext();
    // Use arbFunc=true variant for EvalFunc compatibility
...
TEST_F(BinFHELogicTest_XOR, TestXOR) {
  using namespace lbcrypto;
  ...
  for (auto [a, b] : cases) {
    ...
    auto ct = xor_bits(cryptoContext, ct_a, ct_b);
    LWEPlaintext r;
    cryptoContext->Decrypt(secretKey, ct, &r, 8);
    EXPECT_EQ(r, (a ^ b) ? 1 : 0) << "XOR(" << a << "," << b << ")";
  }
}

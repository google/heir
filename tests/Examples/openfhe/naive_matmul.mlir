// The end to end mlir-to-openfhe-ckks pipeline will automatically rewrite a
// matrix multiplication into a Halevi-Shoup diagonalized matrix multiplication.
// This example is preserved to show a speedup of that pass from a naive
// implementation. We start at the CKKS dialect to avoid the automatic
// diagonalization.

!Z463187969_i32_ = !mod_arith.int<463187969 : i32>
#ring_Z463187969_i32_1_x1024_ = #polynomial.ring<coefficientType = !Z463187969_i32_, polynomialModulus = <1 + x**1024>>
!rlwe_pt = !lwe.rlwe_plaintext<encoding = #lwe.polynomial_evaluation_encoding<cleartext_start = 32, cleartext_bitwidth = 32>, ring = #ring_Z463187969_i32_1_x1024_, underlying_type = f32>
!rlwe_ct = !lwe.rlwe_ciphertext<encoding = #lwe.polynomial_evaluation_encoding<cleartext_start = 32, cleartext_bitwidth = 32>, rlwe_params = <ring = #ring_Z463187969_i32_1_x1024_>, underlying_type = f32>
module {
  func.func @matmul(%arg0: tensor<1x16x!rlwe_ct>, %arg1: tensor<1x16x!rlwe_ct> {secret.secret}) -> tensor<1x16x!rlwe_ct> {
    %cst = arith.constant dense<""> : tensor<16x16xf32>
    %c0 = arith.constant 0 : index
    %0 = affine.for %arg2 = 0 to 16 iter_args(%arg3 = %arg1) -> (tensor<1x16x!rlwe_ct>) {
      %1 = affine.for %arg4 = 0 to 16 iter_args(%arg5 = %arg3) -> (tensor<1x16x!rlwe_ct>) {
        %extracted = tensor.extract %arg0[%c0, %arg4] : tensor<1x16x!rlwe_ct>
        %extracted_0 = tensor.extract %cst[%arg4, %arg2] : tensor<16x16xf32>
        %extracted_1 = tensor.extract %arg5[%c0, %arg2] : tensor<1x16x!rlwe_ct>
        %2 = lwe.rlwe_encode %extracted_0 {encoding = #lwe.polynomial_evaluation_encoding<cleartext_start = 32, cleartext_bitwidth = 32>, ring = #ring_Z463187969_i32_1_x1024_} : f32 -> !rlwe_pt
        %3 = ckks.mul_plain %extracted, %2 : (!rlwe_ct, !rlwe_pt) -> !rlwe_ct
        %4 = ckks.add %extracted_1, %3 : !rlwe_ct
        %inserted = tensor.insert %4 into %arg5[%c0, %arg2] : tensor<1x16x!rlwe_ct>
        affine.yield %inserted : tensor<1x16x!rlwe_ct>
      }
      affine.yield %1 : tensor<1x16x!rlwe_ct>
    }
    return %0 : tensor<1x16x!rlwe_ct>
  }
}

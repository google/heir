...
func TestConv2dChannelPadding(t *testing.T) {
	...
	ctZeros := Conv2d_channel_pad__encrypt__zeros(evaluator, params, ecd, enc)
	startPre := time.Now()
	...
	start := time.Now()
	resultCt := Conv2d_channel_pad__preprocessed(evaluator, params, ecd, ct0, ctZeros, filterPlains)

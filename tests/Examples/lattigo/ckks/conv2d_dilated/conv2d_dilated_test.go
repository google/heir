...
func TestConv2DDilated(t *testing.T) {
	...
	startPre := time.Now()
	filterPlains, linearTransforms := conv2d_dilated_utils.Conv2d_dilated__preprocessing(params, ecd)
	t.Logf("Preprocessing took %s", time.Since(startPre))

	start := time.Now()
	resultCt := Conv2d_dilated__preprocessed(evaluator, params, ecd, ct0, filterPlains, linearTransforms)

...
!poly = !_polynomial.polynomial<<cmod=33538049, ideal=#_polynomial.polynomial<1 + x**1024>>>

// CHECK-LABEL:  @test_elementwise
// CHECK: {{.*}} -> [[T:tensor<2x!_polynomial.*33538049.*]] {
func.func @test_elementwise(%arg0: tensor<2x!poly>, %arg1: tensor<2x!poly>) ->  tensor<2x!poly> {
  // CHECK: [[EMPTY:%.+]] = tensor.empty() : [[T:tensor<2x!_polynomial.*33538049.*]]
  // CHECK: [[LOOP:%.+]] = affine.for [[I:%.+]] = 0 to 2 iter_args([[T0:%.+]] = [[EMPTY]]) -> ([[T]]) {
    // CHECK: [[A:%.+]] = tensor.extract %arg0[[[I]]] : [[T]]
    // CHECK: [[B:%.+]] = tensor.extract %arg1[[[I]]] : [[T]]
    // CHECK: [[S:%.+]] = _polynomial.add([[A]], [[B]]) : [[P:!_polynomial.*33538049.*]]
    // CHECK-NOT: _polynomial.add{{.*}} : [[T]]
    // CHECK: [[R:%.+]] = tensor.insert [[S]] into [[T0]][[[I]]] : [[T]]
    // CHECK: affine.yield [[R]] : [[T]]
  %0 = _polynomial.add(%arg0, %arg1) : tensor<2x!poly>
  // CHECK: return [[LOOP]] : [[T]]
...
// CHECK: {{.*}} -> [[T:tensor<2x3x!_polynomial.*33538049.*]] {
func.func @test_elementwise_multidim(%arg0: tensor<2x3x!poly>, %arg1: tensor<2x3x!poly>) ->  tensor<2x3x!poly> {
  %0 = _polynomial.add(%arg0, %arg1) : tensor<2x3x!poly>
  return %0 :  tensor<2x3x!poly>
  // CHECK: [[EMPTY:%.+]] = tensor.empty() : [[T:tensor<2x3x!_polynomial.*33538049.*]]
  // CHECK: [[LOOP:%.+]] = affine.for [[I:%.+]] = 0 to 2 iter_args([[T0:%.+]] = [[EMPTY]]) -> ([[T]]) {

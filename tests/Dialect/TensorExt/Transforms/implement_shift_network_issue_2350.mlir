...
module {
  ...
  // CHECK: %[[v0:.*]] = tensor.empty() : tensor<1x1024xf32>
  // CHECK: secret.generic(%[[arg0]]
  // CHECK-NEXT:  ^body(%[[input0:.*]]: tensor<1x1024xf32>):
  // CHECK: %[[extracted:.*]] = tensor.extract_slice %[[input0]][0, 0] [1, 1024] [1, 1] : tensor<1x1024xf32> to tensor<1024xf32>
  // CHECK: %[[inserted:.*]] = tensor.insert_slice %[[extracted]] into %[[v0]][0, 0] [1, 1024] [1, 1]
  // CHECK: secret.yield %[[inserted]]

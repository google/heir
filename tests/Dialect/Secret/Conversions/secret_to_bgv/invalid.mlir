...
module attributes {bgv.schemeParam = #bgv.scheme_param<logN = 14, Q = [67239937, 17179967489, 17180262401, 17180295169, 17180393473, 70368744210433], P = [70368744570881, 70368744701953], plaintextModulus = 65537>} {
  // expected-error@below {{expected secret types to be tensors with last dimension less than or equal to ring parameter}}
  func.func @test_invalid_dimension(%arg0 : !secret.secret<tensor<20000xi1>> {mgmt.mgmt = #mgmt}) -> (!secret.secret<tensor<20000xi1>> {mgmt.mgmt = #mgmt}) {
    return %arg0 : !secret.secret<tensor<20000xi1>>
  }
}

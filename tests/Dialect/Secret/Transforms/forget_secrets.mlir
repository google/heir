...
func.func @test_conceal_then_generic(%value : i32) {
  ...
  %Y = secret.conceal %value : i32 -> !secret.secret<i32>
  %Z = secret.generic ins(%X: i32, %Y: !secret.secret<i32>) {
    ^bb0(%x: i32, %y: i32) :
...
func.func @test_add_two_secrets(
  ...
  %out = secret.generic ins(%s1: !secret.secret<i32>, %s2: !secret.secret<i32>) {
    ^bb0(%x: i32, %y: i32) :
      %d = arith.addi %x, %y: i32
      secret.yield %d : i32
    } -> (!secret.secret<i32>)
...
func.func @test_compose_generic(
    %s1 : !secret.secret<i32>,
    %s2 : !secret.secret<i32>) -> !secret.secret<i32> {
  %0 = secret.generic ins(%s1: !secret.secret<i32>, %s2:  !secret.secret<i32>) {
    ^bb0(%x: i32, %y: i32) :
      %d = arith.addi %x, %y: i32
      secret.yield %d : i32
    } -> (!secret.secret<i32>)
  %1 = secret.generic ins(%s1: !secret.secret<i32>, %0: !secret.secret<i32>) {
    ^bb0(%x: i32, %y: i32) :

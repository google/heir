...
func.func @test_lower_mul_scalar_prime_cmod(%arg0: !poly_prime) -> !poly_prime {
  // CHECK: [[C2:%.*]] = arith.constant 2 : i32
  // CHECK: [[C2EXT:%.+]] = arith.extsi [[C2]] : i32 to i33
  // CHECK: [[C2RED:%.+]] = mod_arith.reduce [[C2EXT]] {modulus = 4294967291 : i64} : i33
  // CHECK: [[C2TRUNC:%.+]] = arith.trunci [[C2RED]] : i33 to i32
  %c2 = arith.constant 2 : i32
  // CHECK: [[SPLAT:%.*]] = tensor.splat [[C2TRUNC]] : [[T]]

...
func.func @test_lower_add_power_of_two_cmod() -> !poly {
  ...
  // CHECK-NEXT: [[ADD:%.+]] = mod_arith.add [[X]], [[Y]] {modulus = 4294967296 : i64}
...
func.func @test_lower_add_no_cmod() -> !poly_no {
  ...
  // CHECK-NEXT: [[ADD:%.+]] = mod_arith.add [[X]], [[Y]] {modulus = 4294967296 : i34}
...
func.func @test_lower_add_prime_cmod() -> !poly_prime {
  ...
  // CHECK-NOT: polynomial.from_tensor
  // CHECK: [[XEXT:%.+]] = arith.extsi [[X]] : [[TCOEFF]] to [[T:tensor<1024xi32>]]
  // CHECK: [[XEXT2:%.+]] = arith.extsi [[XEXT]] : [[T]] to [[TEXT:tensor<1024xi33>]]
  // CHECK: [[XRED:%.+]] = mod_arith.reduce [[XEXT2]] {modulus = 4294967291 : i64} : [[TEXT]]
  // CHECK: [[XTRUNC:%.+]] = arith.trunci [[XRED]] : [[TEXT]] to [[T]]
  %poly0 = polynomial.from_tensor %coeffs1 : tensor<1024xi31> -> !poly_prime
  // CHECK-NOT: polynomial.from_tensor
  // CHECK: [[YEXT:%.+]] = arith.extsi [[Y]] : [[TCOEFF]] to [[T]]
  // CHECK: [[YEXT2:%.+]] = arith.extsi [[YEXT]] : [[T]] to [[TEXT]]
  // CHECK: [[YRED:%.+]] = mod_arith.reduce [[YEXT2]] {modulus = 4294967291 : i64} : [[TEXT]]
  // CHECK: [[YTRUNC:%.+]] = arith.trunci [[YRED]] : [[TEXT]] to [[T]]
  %poly1 = polynomial.from_tensor %coeffs2 : tensor<1024xi31> -> !poly_prime

  // CHECK: [[ADD_RESULT:%.+]] = mod_arith.add [[XTRUNC]], [[YTRUNC]] {modulus = 4294967291 : i64} : [[T]]

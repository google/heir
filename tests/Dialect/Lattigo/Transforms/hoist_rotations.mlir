...
module {
  func.func @simple_sum(%evaluator: !evaluator, %ct: !ct) -> !ct {
    ...
    %ct_0 = lattigo.ckks.rotate_new %evaluator, %ct {static_shift = 16} : (!evaluator, !ct) -> !ct
    %ct_1 = lattigo.ckks.add_new %evaluator, %ct, %ct_0 : (!evaluator, !ct, !ct) -> !ct
    %ct_2 = lattigo.ckks.rotate_new %evaluator, %ct {static_shift = 8} : (!evaluator, !ct) -> !ct
    %ct_3 = lattigo.ckks.add_new %evaluator, %ct_1, %ct_2 : (!evaluator, !ct, !ct) -> !ct
    %ct_4 = lattigo.ckks.rotate_new %evaluator, %ct {static_shift = 5} : (!evaluator, !ct) -> !ct
    %ct_5 = lattigo.ckks.add_new %evaluator, %ct_3, %ct_4 : (!evaluator, !ct, !ct) -> !ct
    %ct_6 = lattigo.ckks.rotate_new %evaluator, %ct {static_shift = 12} : (!evaluator, !ct) -> !ct

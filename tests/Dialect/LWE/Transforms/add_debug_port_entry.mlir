// RUN: heir-opt --mlir-print-local-scope --lwe-add-debug-port=entry-function=foo %s | FileCheck %s
...
func.func @bar(%arg0: !ty) -> !ty {
  return %arg0 : !ty
}

// CHECK: func.func @foo(%{{.*}}: !lwe.lwe_secret_key{{.*}}, %{{.*}}: !lwe.lwe_ciphertext{{.*}})
// CHECK: func.func @bar(%{{.*}}: !lwe.lwe_ciphertext{{.*}})

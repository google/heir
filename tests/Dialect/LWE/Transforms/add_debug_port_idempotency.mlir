// RUN: heir-opt --mlir-print-local-scope --lwe-add-debug-port=insert-debug-after-every-op=true --lwe-add-debug-port=insert-debug-after-every-op=true %s | FileCheck %s

// CHECK: func.func private @__heir_debug_1(!lwe.lwe_secret_key<{{.*}}>, !lwe.lwe_ciphertext<{{.*}}x{{.*}}64{{.*}}>)
...
#ring_rns_L0_1_x64_ = #polynomial.ring<coefficientType = !rns_L0_, polynomialModulus = <1 + x**64>>
#ciphertext_space_L0_2_ = #lwe.ciphertext_space<ring = #ring_rns_L0_1_x64_, encryption_type = lsb>
!ty2 = !lwe.lwe_ciphertext<plaintext_space = #plaintext_space, ciphertext_space = #ciphertext_space_L0_2_, key = #key, modulus_chain = #modulus_chain_L5_C0_>
!sk1 = !lwe.lwe_secret_key<key = #key, ring = #ring_rns_L0_1_x32_>

func.func private @__heir_debug_0(!sk1, !ty)

// CHECK: func.func private @__heir_debug_0(!lwe.lwe_secret_key<{{.*}}>, !lwe.lwe_ciphertext<{{.*}}>)

// CHECK: func.func @mixed_test
// CHECK-SAME: (%[[sk:[^:]*]]: [[sk_ty:[^,]*]],
// CHECK-SAME: %[[arg1:[^:]*]]: [[ty1:[^,]*]],
// CHECK-SAME: %[[arg2:[^:]*]]: [[ty2:[^)]*]])
func.func @mixed_test(%arg0: !sk1, %arg1: !ty, %arg2: !ty2) -> (!ty, !ty2) {
  // CHECK: call @__heir_debug_1(%[[sk]], %[[arg2]])
  // CHECK: call @__heir_debug_0(%[[sk]], %[[arg1]])

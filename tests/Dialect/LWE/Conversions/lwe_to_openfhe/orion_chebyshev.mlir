...
// CHECK: func.func @chebyshev
// CHECK-NOT: orion.chebyshev
// CHECK: %[[CHEB:.+]] = openfhe.chebyshev %[[CC:.+]], %[[CT:.+]] {coefficients = [0.000000e+00, 7.500000e-01, 0.000000e+00, 2.500000e-01], domain_end = 1.000000e+00 : f64, domain_start = -1.000000e+00 : f64} : (!cc, !ct) -> !ct
// CHECK: return %[[CHEB]]
...
module attributes {scheme.ckks, ckks.schemeParam = #ckks.scheme_param<logN = 16, Q = [36028797019488257, 1099512938497, 1099510054913, 1099507695617, 1099515691009, 1099516870657, 1099506515969, 1099504549889, 1099503894529, 1099503370241, 1099502714881], P = [2305843009211596801, 2305843009210023937, 2305843009208713217], logDefaultScale = 40>} {
  func.func @chebyshev(%ct: !ct_L10) -> !ct_L10 {
    %0 = orion.chebyshev %ct {coefficients = [0.0, 0.75, 0.0, 0.25], domain_end = 1.000000e+00 : f64, domain_start = -1.000000e+00 : f64} : (!ct_L10) -> !ct_L10
    return %0 : !ct_L10
  }
}

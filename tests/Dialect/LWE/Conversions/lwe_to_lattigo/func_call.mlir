...
// CHECK: [[evType:.*]] = !lattigo.ckks.evaluator
module @jit_func attributes {backend.openfhe, ckks.schemeParam = #ckks.scheme_param<logN = 13, Q = [36028797018652673, 35184372121601], P = [1152921504606994433], logDefaultScale = 45>, scheme.ckks} {
  ...
  func.func public @mnist(%arg0: tensor<512x784xf32>, %arg1: tensor<1x!ct_L1> {tensor_ext.original_type = #tensor_ext.original_type<originalType = tensor<1x784xf32>, layout = #layout2>}) -> (tensor<1x!ct_L1> {tensor_ext.original_type = #original_type}) {
...
// CHECK-DAG: [[paramType:.*]] = !lattigo.ckks.parameter
// CHECK-DAG: [[enType:.*]] = !lattigo.ckks.encoder
module @jit_func attributes {backend.openfhe, ckks.schemeParam = #ckks.scheme_param<logN = 13, Q = [36028797018652673, 35184372121601], P = [1152921504606994433], logDefaultScale = 45>, scheme.ckks} {
  // CHECK: func.func private @_assign_layout_11979326689855340354(tensor<512x784xf32>) -> tensor<512x1024xf32>
  // CHECK: func.func @mnist_preprocessing(%[[param:.*]]: [[paramType]], %[[en:.*]]: [[enType]]
  ...
  func.func public @mnist(%arg0: tensor<512x784xf32>, %arg1: tensor<1x!ct_L1> {tensor_ext.original_type = #tensor_ext.original_type<originalType = tensor<1x784xf32>, layout = #layout2>}) -> (tensor<1x!ct_L1> {tensor_ext.original_type = #original_type}) {
    %0 = call @mnist_preprocessing() : () -> !pt
    %1 = call @mnist__preprocessed(%arg1, %0) : (tensor<1x!ct_L1>, !pt) -> tensor<1x!ct_L1>
    return %1 : tensor<1x!ct_L1>
  }
}

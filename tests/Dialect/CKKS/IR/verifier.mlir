...
func.func @test_extract_verification_failure(%input: !ct) -> !ct {
  ...
  // expected-error@+1 {{'ckks.extract' op input RLWE ciphertext type must have a ranked tensor as its underlying_type, but found 'i3'}}
  %ext = ckks.extract %input, %offset : (!ct, index) -> !ct
  return %ext : !ct
}
...
func.func @test_extract_verification_failure(%input: !ct_tensor) -> !ct_scalar {
  %offset = arith.constant 4 : index
  // expected-error@+1 {{'ckks.extract' op output RLWE ciphertext's underlying_type must be the element type of the input ciphertext's underlying tensor type, but found tensor type 'tensor<64xi16>' and output type 'i3'}}
  %ext = ckks.extract %input, %offset : (!ct_tensor, index) -> !ct_scalar

...
}
{
  ...
  // CHECK-SAME: [[ksk:%.+]]: tensor<2x!ct_L6>) -> (!ringelt4, !ringelt4) {
  func.func @test_keyswitch_partialpart(%x: !ringelt_L5, %arg0: tensor<2x!ct_L7>) -> (!ringelt_L5, !ringelt_L5) {
    // CHECK-DAG: [[C0:%.+]] = arith.constant 0 : index
    // CHECK-DAG: [[C1:%.+]] = arith.constant 1 : index
    ...
    // CHECK-DAG: [[ksk0:%.+]] = tensor.extract [[ksk]][[[C0]]] : tensor<2x!ct_L6>
    // CHECK-DAG: [[ksk1:%.+]] = tensor.extract [[ksk]][[[C1]]] : tensor<2x!ct_L6>
    // CHECK-DAG: [[ksk2:%.+]] = tensor.extract [[ksk]][[[C2]]] : tensor<2x!ct_L6>
    ...
    %constTerm, %linearTerm = ckks.key_switch_inner %x, %arg0 : (!ringelt_L5, tensor<2x!ct_L7>) -> (!ringelt_L5, !ringelt_L5)

...
module {
  // CHECK-LABEL: func.func @test_relin(
  ...
  func.func @test_relin(%ct: !ct_L1_D3, %arg0: tensor<10x!ct_L1>) -> !ct_L1 {
    ...
    // CHECK-DAG: [[ksOutput:%.+]] = ckks.key_switch_inner [[C2]], [[ksk]]
    // CHECK-DAG: [[subct:%.+]] = ckks.ct_from_coeffs [[C0]], [[C1]]
    // CHECK-DAG: [[result:%.+]] = ckks.add [[ksOutput]], [[subct]]

...
!ct_D4 = !lwe.lwe_ciphertext<plaintext_space = #plaintext_space, ciphertext_space = #ciphertext_space_L1_D4_, key = #key, modulus_chain = #modulus_chain_L5_C1_>
!ct_L0 = !lwe.lwe_ciphertext<plaintext_space = #plaintext_space, ciphertext_space = #ciphertext_space_L0_, key = #key, modulus_chain = #modulus_chain_L5_C0_>
...
module {
  ...
  func.func @test_ops(%x : !ct, %y : !ct, %z : !pt) -> (!ct, !ct, !ct, !ct_D3, !ct, !ct, !ct, !ct) {
    ...
    // CHECK: %[[v8:.*]] = openfhe.mul_plain [[C]], %[[x8:.*]], %[[z8:.*]]: ([[S]], [[T]], [[P]]) -> [[T]]
    %mul_plain = bgv.mul_plain %x, %z : (!ct, !pt) -> !ct
  ...
  func.func @test_rotate_dynamic(%x : !ct) -> !ct {
    ...
    %rot = bgv.rotate_cols %x, %c4 : i32 : !ct
    return %rot : !ct
  }

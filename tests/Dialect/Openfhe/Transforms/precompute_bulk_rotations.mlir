...
module {
  func.func @simple_sum(%cc: !cc, %ct: !ct) -> !ct {
    ...
    %ct_0 = openfhe.rot %cc, %ct {index = 16 : index} : (!cc, !ct) -> !ct
    %ct_1 = openfhe.add %cc, %ct, %ct_0 : (!cc, !ct, !ct) -> !ct
    %ct_2 = openfhe.rot %cc, %ct {index = 8 : index} : (!cc, !ct) -> !ct
    %ct_3 = openfhe.add %cc, %ct_1, %ct_2 : (!cc, !ct, !ct) -> !ct
    %ct_4 = openfhe.rot %cc, %ct {index = 5 : index} : (!cc, !ct) -> !ct
    %ct_5 = openfhe.add %cc, %ct_3, %ct_4 : (!cc, !ct, !ct) -> !ct
    %ct_6 = openfhe.rot %cc, %ct {index = 12 : index} : (!cc, !ct) -> !ct

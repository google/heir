...
module {
  func.func @main(%arg0: tensor<1x1xi8> {iree.identifier = "serving_default_dense_input:0", secret.secret, tf_saved_model.index_path = ["dense_input"]}) -> (tensor<1x1xi32> {iree.identifier = "StatefulPartitionedCall:0", tf_saved_model.index_path = ["dense_2"]}) attributes {tf_saved_model.exported_names = ["serving_default"]} {
    ...
    %cst_0 = arith.constant dense<[[0, 0, 5438, 5515, 1352, 1500, 4152, 84, 3396, 0, 1981, 5581, 0, 6964, 3407, 7217]]> : tensor<1x16xi32>
    ...
    %2 = linalg.quantized_matmul ins(%arg0, %cst, %c0_i32, %c0_i32 : tensor<1x1xi8>, tensor<1x16xi8>, i32, i32) outs(%cst_0 : tensor<1x16xi32>) -> tensor<1x16xi32>
    %4 = linalg.quantized_matmul ins(%2, %cst_3, %c0_i32, %c0_i32 : tensor<1x16xi32>, tensor<16x16xi8>, i32, i32) outs(%cst_1 : tensor<1x16xi32>) -> tensor<1x16xi32>

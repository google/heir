// RUN: heir-opt --tensor-linalg-to-affine-loops %s | FileCheck %s

#map = affine_map<(d0, d1) -> (d0, d1)>
#map1 = affine_map<(d0, d1) -> (d1 mod 16, (d0 + d1) mod 16)>

// CHECK: func @assign_layout
func.func @assign_layout() -> tensor<16x16xf32> {
  // CHECK: [[output_init:%[^ ]*]] = arith.constant dense<0.000000e+00>
  %cst = arith.constant dense<0.000000e+00> : tensor<16x16xf32>
  // CHECK: [[matrix:%[^ ]*]] = arith.constant dense<"0x
  %cst_1 = arith.constant dense<""> : tensor<16x16xf32>
  // CHECK: [[outer_loop:%[^ ]*]] = affine.for [[outer_iv:%[^ ]*]] = 0 to 16 iter_args([[outer_iter:[^ ]*]] = [[output_init]])
  // CHECK: [[inner_loop:%[^ ]*]] = affine.for [[inner_iv:%[^ ]*]] = 0 to 16 iter_args([[inner_iter:[^ ]*]] = [[outer_iter]])

  // CHECK: [[extracted:%[^ ]*]] = tensor.extract [[matrix]]
  // CHECK: [[carried:%[^ ]*]] = tensor.insert [[extracted]] into [[inner_iter]]

  // CHECK: affine.yield [[carried]]
  // CHECK: }

  // CHECK: affine.yield [[inner_loop]]
  // CHECK: }
  %0 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel"]} ins(%cst_1 : tensor<16x16xf32>) outs(%cst : tensor<16x16xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  } -> tensor<16x16xf32>

  // CHECK-NEXT: return [[outer_loop]]
  return %0 : tensor<16x16xf32>
}

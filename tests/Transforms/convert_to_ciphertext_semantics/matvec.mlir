...
#new_layout = #tensor_ext.new_layout<"{ [i0] -> [ct, slot] : ct = 0 and (-i0 + slot) mod 16 = 0 and 0 <= i0 <= 15 and 0 <= slot <= 1023 }">
#new_layout1 = #tensor_ext.new_layout<"{ [i0, i1] -> [ct, slot] : (i0 - i1 + ct) mod 16 = 0 and (-i0 + slot) mod 16 = 0 and 0 <= i0 <= 15 and 0 <= i1 <= 15 and 0 <= ct <= 15 and 0 <= slot <= 1023 }">
module {
...
// CHECK: arith.addf
// CHECK: arith.addf
// CHECK: tensor.expand_shape {{.*}} : tensor<1024xf32> into tensor<1x1024xf32>
// CHECK: secret.yield
// CHECK: return
  func.func @square(%arg0: !secret.secret<tensor<16xf32>> {tensor_ext.layout = #new_layout}) -> (!secret.secret<tensor<16xf32>> {tensor_ext.layout = #new_layout}) {
    ...
    %0 = secret.generic(%arg0: !secret.secret<tensor<16xf32>> {tensor_ext.layout = #new_layout}) {
    ^body(%input0: tensor<16xf32>):
      %1 = tensor_ext.assign_layout %cst_0 {layout = #new_layout1, tensor_ext.layout = #new_layout1} : tensor<16x16xf32>
      %2 = tensor_ext.assign_layout %cst {layout = #new_layout, tensor_ext.layout = #new_layout} : tensor<16xf32>
      %3 = linalg.matvec {secret.kernel = #kernel, tensor_ext.layout = #new_layout} ins(%1, %input0 : tensor<16x16xf32>, tensor<16xf32>) outs(%2 : tensor<16xf32>) -> tensor<16xf32>
      secret.yield %3 : tensor<16xf32>
    } -> (!secret.secret<tensor<16xf32>> {tensor_ext.layout = #new_layout})
...
// CHECK: return
#kernel = #secret.kernel<name = "MatvecDiagonal", force = false>
#new_layout = #tensor_ext.new_layout<"{ [i0] -> [ct, slot] : ct = 0 and (-i0 + slot) mod 16 = 0 and 0 <= i0 <= 9 and 0 <= slot <= 1023 }">
#new_layout1 = #tensor_ext.new_layout<"{ [i0] -> [ct, slot] : ct = 0 and (-i0 + slot) mod 16 = 0 and 0 <= i0 <= 15 and 0 <= slot <= 1023 }">
#new_layout2 = #tensor_ext.new_layout<"{ [i0, i1] -> [ct, slot] : (i0 - i1 + ct) mod 16 = 0 and (-i0 + slot) mod 16 = 0 and 0 <= i0 <= 9 and 0 <= i1 <= 15 and 0 <= ct <= 15 and 0 <= slot <= 1023 }">
module {
  func.func @squat(%arg0: !secret.secret<tensor<16xf32>> {tensor_ext.layout = #new_layout1}) -> (!secret.secret<tensor<10xf32>> {tensor_ext.layout = #new_layout}) {
    ...
    %0 = secret.generic(%arg0: !secret.secret<tensor<16xf32>> {tensor_ext.layout = #new_layout1}) {
    ^body(%input0: tensor<16xf32>):
      %1 = tensor_ext.assign_layout %cst_0 {layout = #new_layout2, tensor_ext.layout = #new_layout2} : tensor<10x16xf32>
      %2 = tensor_ext.assign_layout %cst {layout = #new_layout, tensor_ext.layout = #new_layout} : tensor<10xf32>
      %3 = linalg.matvec {secret.kernel = #kernel, tensor_ext.layout = #new_layout} ins(%1, %input0 : tensor<10x16xf32>, tensor<16xf32>) outs(%2 : tensor<10xf32>) -> tensor<10xf32>
      secret.yield %3 : tensor<10xf32>
    } -> (!secret.secret<tensor<10xf32>> {tensor_ext.layout = #new_layout})

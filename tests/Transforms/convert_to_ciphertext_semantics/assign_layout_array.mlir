...
module {
  ...
  // CHECK-SAME: %[[CST:.*]]: tensor<4xi16>
  func.func @assign_layout_array_i16(%cst: tensor<4xi16>) -> (!secret.secret<tensor<4xi16>> {tensor_ext.layout = #composed}) {
...
module {
  ...
  // CHECK-SAME: %[[CST:.*]]: tensor<2x2xi32>
  func.func @assign_layout_array_3step(%cst: tensor<2x2xi32>) -> (!secret.secret<tensor<2x2xi32>> {tensor_ext.layout = #composed}) {
    // CHECK: %[[GEN:.*]] = secret.generic()

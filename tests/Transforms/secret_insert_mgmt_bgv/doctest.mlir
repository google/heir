// RUN: heir-opt --secret-insert-mgmt-bgv %s | FileCheck %s

// CHECK: func.func @func
// CHECK:      %[[GENERIC:.*]] = secret.generic(%{{.*}}, %{{.*}}) {arg0 = {mgmt.mgmt = #mgmt.mgmt<level = 1>}, arg1 = {mgmt.mgmt = #mgmt.mgmt<level = 1>}}
// CHECK:      ^bb0(%[[ARG0:.*]]: i16, %[[ARG1:.*]]: i16):

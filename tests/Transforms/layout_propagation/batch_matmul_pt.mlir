// RUN: heir-opt --layout-propagation=min-slot-count=1024 --mlir-print-local-scope %s | FileCheck %s
...
module {
  // CHECK: func @batch_matmul_broadcast
  // CHECK-NOT: tensor_ext.convert_layout
  // CHECK: linalg.batch_matmul
  // CHECK-SAME: #secret.kernel<name = "BatchMatmulTricyclicDiagonal"
  ...
  func.func @batch_matmul_ptct(%arg0: tensor<2x3x5xf32>, %arg1: !secret.secret<tensor<2x5x7xf32>> {tensor_ext.layout = #tricyclic_2_5_7}) -> !secret.secret<tensor<2x3x7xf32>> {
    ...
    ^body(%input1: tensor<2x5x7xf32>):
      %1 = linalg.batch_matmul ins(%arg0, %input1 : tensor<2x3x5xf32>, tensor<2x5x7xf32>) outs(%cst : tensor<2x3x7xf32>) -> tensor<2x3x7xf32>
      secret.yield %1 : tensor<2x3x7xf32>
    } -> !secret.secret<tensor<2x3x7xf32>>
    return %0 : !secret.secret<tensor<2x3x7xf32>>
  }

  // CHECK: func @batch_matmul_unit_dim
  // CHECK: linalg.batch_matmul
  // CHECK-SAME: #secret.kernel<name = "BatchMatmulTricyclicDiagonal"
  func.func @batch_matmul_unit_dim(%arg0: !secret.secret<tensor<1x3x5xf32>>, %arg1: tensor<1x5x7xf32>) -> !secret.secret<tensor<1x3x7xf32>> {
    %cst = arith.constant dense<0.000000e+00> : tensor<1x3x7xf32>
    %0 = secret.generic(%arg0: !secret.secret<tensor<1x3x5xf32>>) {
    ^body(%input0: tensor<1x3x5xf32>):
      %1 = linalg.batch_matmul ins(%input0, %arg1 : tensor<1x3x5xf32>, tensor<1x5x7xf32>) outs(%cst : tensor<1x3x7xf32>) -> tensor<1x3x7xf32>
      secret.yield %1 : tensor<1x3x7xf32>
    } -> !secret.secret<tensor<1x3x7xf32>>
    return %0 : !secret.secret<tensor<1x3x7xf32>>
  }
}

...
// CHECK-DAG: #[[layout:.*]] = #tensor_ext.layout<"{ [i0, i1, i2, i3] -> [ct, slot] : i0 = 0 and ct = 0 and (-25i1 - 5i2 - i3 + slot) mod 128 = 0 and 0 <= i1 <= 3 and 0 <= i2 <= 4 and 0 <= i3 <= 1023 - 25i1 - 5i2 and i3 <= 4 and 0 <= slot <= 1023 and 1024*floor((-128 + 25i1 + 5i2 + i3)/1024) <= -1024 + 25i1 + 5i2 + i3 }">
// CHECK-DAG: #[[layout3:.*]] = #tensor_ext.layout<"{ [i0, i1, i2, i3] -> [ct, slot] : exists (e1, e2, e3: i0 = 0 and (-25i1 - 5i2 - i3 + slot) mod 128 = 0 and 0 <= i1 <= 3 and 0 <= i2 <= 4 and 0 <= i3 <= 1023 - 25i1 - 5i2 and i3 <= 4 and 0 <= slot <= 99 and 1024*floor((-128 + 25i1 + 5i2 + i3)/1024) <= -1024 + 25i1 + 5i2 + i3 and 0 <= e1 <= 4 and slot <= 2e2 <= 3 + slot and -4 + 26slot - 5e1 <= 50e2 <= 26slot - 5e1 and -1 - slot + 4e1 + 2e2 <= 2e3 <= -slot + 4e1 + 2e2 and -1 - 51slot + 10e1 + 100e2 <= 10e3 <= -51slot + 10e1 + 100e2) }">
...
func.func @conv2d_nchw(%arg0: !secret.secret<tensor<1x1x10x10xf32>>) -> !secret.secret<tensor<1x4x5x5xf32>> {
  ...
  ^body(%input0: tensor<1x1x10x10xf32>):
    ...
    // CHECK-SAME: tensor_ext.layout = #[[layout3]]
    ...
    // CHECK: secret.yield
    // CHECK-NEXT: {tensor_ext.layout = #[[layout]]}

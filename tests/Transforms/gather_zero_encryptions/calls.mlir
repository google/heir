...
// CHECK-SAME: (%[[CC:.*]]: !cc, %[[PK:.*]]: !pk) -> memref<2x!ct> attributes {heir.interface = {func_name = "compute", roles = ["client.encrypt_zero"]}}
// CHECK-NEXT: %[[ALLOC:.*]] = memref.alloc() : memref<2x!ct>
func.func @compute__encrypt__zero__0(%cc: !cc, %pk: !pk) -> !ct attributes {heir.interface = {func_name = "compute", index = 0 : i64, roles = ["client.encrypt_zero"]}} {
  %0 = builtin.unrealized_conversion_cast %cc : !cc to !ct
  return %0 : !ct
}

func.func @compute__encrypt__zero__1(%cc: !cc, %pk: !pk) -> !ct attributes {heir.interface = {func_name = "compute", index = 1 : i64, roles = ["client.encrypt_zero"]}} {

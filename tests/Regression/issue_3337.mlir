...
module attributes {backend.lattigo, ckks.schemeParam = #ckks.scheme_param<logN = 13, Q = [36028797018652673, 35184372121601], P = [1152921504606994433], logDefaultScale = 45>, scheme.actual_slot_count = 4096 : i64, scheme.ckks, scheme.requested_slot_count = 1024 : i64} {
  func.func @rotate512__preprocessed(%evaluator: !evaluator, %param: !param, %encoder: !encoder, %arg0: tensor<1x!ct>) -> tensor<1x!ct> attributes {heir.interface = {func_name = "rotate512", roles = ["client.preprocessed"]}} {
  ...
  func.func @rotate512__decrypt__result0(%evaluator: !evaluator, %param: !param, %encoder: !encoder, %decryptor: !decryptor, %arg0: tensor<1x!ct>) -> tensor<1024xf32> attributes {heir.interface = {func_name = "rotate512", index = 0 : i64, roles = ["client.decrypt"]}} {
    %c0 = arith.constant 0 : index

...
module attributes {
  ...
  // The dead low path leaves an L0 allocation at consumed depth 2. Applying
  // the prepared transform after a bootstrap produces L1 at consumed depth 1.
  // The L0 allocation must not become the destination of that result.
  ...
  // CHECK: %[[HIGH:.*]] = lattigo.ckks.rescale_new {{.*}}%[[APPLY]]
  // CHECK: lattigo.ckks.add {{.*}}%[[HIGH]], %[[HIGH]], %[[HIGH]]

...
@patch("requests.get")
def test_get_pypi_versions_success(mock_get):
...
@patch("requests.get")
def test_get_pypi_versions_failure(mock_get):
  mock_response = MagicMock()
...
def test_get_next_dev_version_none_today(mock_datetime, mock_get_pypi):
  ...
  assert next_version == "2026.04.01.dev0"


@patch("scripts.get_version.get_pypi_versions")
@patch("datetime.datetime")
def test_get_next_dev_version_exists_today(mock_datetime, mock_get_pypi):
  mock_datetime.now.return_value.strftime.return_value = "2026.04.01"
  mock_get_pypi.return_value = ["2026.04.01.dev0", "2026.04.01.dev1"]

  next_version = get_next_dev_version("heir_py")
  assert next_version == "2026.04.01.dev2"


@patch("scripts.get_version.get_pypi_versions")
@patch("datetime.datetime")
def test_get_next_dev_version_multiple_years(mock_datetime, mock_get_pypi):
  mock_datetime.now.return_value.strftime.return_value = "2026.04.01"
  mock_get_pypi.return_value = ["2025.04.01.dev0", "2026.04.01.dev0"]

  next_version = get_next_dev_version("heir_py")
  assert next_version == "2026.04.01.dev1"


def test_calculate_version_release():
...
def test_calculate_version_workflow_dispatch_tag():
  version, should_publish = calculate_version(
...
def test_calculate_version_workflow_dispatch_main(mock_get_next_dev):
  mock_get_next_dev.return_value = "2026.04.01.dev0"
  version, should_publish = calculate_version(
      "workflow_dispatch", "refs/heads/main", None, "heir_py"
  )
  assert version == "2026.04.01.dev0"
  assert should_publish == "true"


def test_calculate_version_pr():
  version, should_publish = calculate_version(
